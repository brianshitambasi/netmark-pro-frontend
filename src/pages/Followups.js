...
function Followups() {
    ...
    try {
      ...
      if (rescheduleData.daysToAdd) {
        ...
        response = await fetch(`${process.env.REACT_APP_API_URL}/followups/${selectedId}/reschedule`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('token')}` },
          body: JSON.stringify({ nextCallDate: rescheduleData.nextCallDate, reason: rescheduleData.reason })
        });
      }
  ...
  const getPaymentBadge = (status) => {
    ...
    const labels = { paid: '‚úÖ Paid', partial: '‚ö†Ô∏è Partial', pending: '‚è≥ Pending' };
    ...
    <div className="fade-in">
      ...
      <Row className="mb-4">
        <Col md={6}>
          <InputGroup>
            ...
            <Form.Control placeholder="Search by name or phone..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </InputGroup>
        </Col>
      </Row>
      ...
      <Modal show={showModal} onHide={() => { setShowModal(false); setSelectedPackage(null); }} size="lg" centered>
        ...
        <Form onSubmit={handleSubmit}>
          <Modal.Body>
            <Alert variant="info" className="small"><strong>Ì≥ã Select Investment Package</strong> - Choose the package the prospect is interested in</Alert>
            <div className="mb-4">
      ...
      <Modal show={showRescheduleModal} onHide={() => setShowRescheduleModal(false)} centered>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleReschedule}>Confirm</Button>
        </Modal.Footer>
      </Modal>
