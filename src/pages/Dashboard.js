...
import { 
  FaPhone, FaCheckCircle, FaChartLine, FaUsers, FaBullseye, FaWhatsapp, 
  FaClock, FaUserPlus, FaDollarSign, FaRocket, FaBell, FaExclamationTriangle,
  FaThumbsUp, FaStar, FaCalendarCheck, FaArrowUp, FaArrowDown
} from 'react-icons/fa';
...
function Dashboard() {
  ...
  // Prepare chart data
  const getChartData = () => {
    // Sample weekly activity data - in production, this would come from API
    const weeklyData = {
            ...
            <Card className={`border-0 shadow-sm h-100 stat-card stat-card-${card.color}`}>
              <Card.Body className="p-4">
                <div className="d-flex justify-content-between align-items-start mb-3">
                  <div className={`stat-icon bg-${card.color} bg-opacity-10 rounded-3 p-3`}>
                    {card.icon}
                  </div>
            ...
            <Card.Header className="bg-white border-0 d-flex justify-content-between align-items-center py-3">
              ...
              <div className="d-flex gap-2">
                {['week', 'month', 'year'].map(t => (
                  <button
                    key={t}
                    className={`btn btn-sm ${timeframe === t ? 'btn-primary' : 'btn-outline-secondary'}`}
                ...
                stats.goals.map((goal) => (
                  <div key={goal.id} className="mb-3">
                    ...
                    <BootstrapProgressBar 
                      now={goal.progress} 
                      variant={goal.isBehind ? 'warning' : 'success'}
            ...
            <Card.Body>
              <div className="d-grid gap-2">
                {quickActions.map((action, idx) => (
                  <button
                    key={idx}
                    className={`btn btn-outline-${action.color} text-start d-flex align-items-center gap-3 p-3`}
              ...
              <div className="d-flex align-items-start gap-3">
                ...
                <div className="flex-grow-1">
                  <Alert.Heading className="mb-1">
                    âš ï¸ {stats.overdueFollowups.length} Overdue Follow-up{stats.overdueFollowups.length > 1 ? 's' : ''}
                  </Alert.Heading>
